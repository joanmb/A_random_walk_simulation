...
```{r tau}
tau_vect <- which(df['S'] == max(df['S']))
tau <- tail(tau_vect, n = 1) - 1
tau
```

```{r gamma}
gamma_vect <- which(df['S'] == df[1,2])
...
g_tau_gamma <- g +
    geom_point(aes(x = tau, y = df[tau + 1, 2]), colour="black", size = 2.7) +
    geom_point(aes(x = tau, y = 0), colour="black", size = 2.7, shape = 4) +
    geom_segment(aes(x = tau, y = 0, xend = tau, yend = df[tau + 1, 2]),
                 linetype = "dotted", size = 0.2) +
    geom_point(aes(x = gamma, y = df[gamma + 1, 2]), colour="red", size = 2.7) +
